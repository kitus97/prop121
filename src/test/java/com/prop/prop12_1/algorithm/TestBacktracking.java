...
public class TestBacktracking {
    ...
    @Test
    public void testGenerateSolutionValidInput() {
        Pair<Double, List<Pair<Integer, Set<String>>>> result = backTracking.generateSolution(shelf, products, similarityTable);

        assertNotNull(result);
        assertNotNull(result.getLeft());
        assertNotNull(result.getRight());
        assertEquals(3, result.getRight().size());
        assertTrue(result.getLeft() >= 0.0);
    }

    @Test
    public void testGenerateSolutionEmptyProducts() {
        products = new ArrayList<>();

        Pair<Double, List<Pair<Integer, Set<String>>>> result = backTracking.generateSolution(shelf, products, similarityTable);

        assertNotNull(result);
        assertEquals(0.0, result.getLeft());
        assertEquals(3, result.getRight().size());
        for (Pair<Integer, Set<String>> pair : result.getRight()) {
            assertNull(pair.getLeft());
        }
    }

    @Test
    public void testGenerateSolutionUnmatchedRestrictions() {
        products = new ArrayList<>();
